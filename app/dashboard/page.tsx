...
const DashboardIcon = () => (
  ...
  >
    <path
      ...
      strokeLinejoin="round"
      strokeWidth={2}
      d="M3 7h7v7H3zM13 3h8v5h-8zM13 12h8v9h-8zM3 17h7v4H3z"
    />
  </svg>
);
...
export default function Dashboard() {
  ...
  const updateStatus = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/applications/${statusApp.id}/status`,
        {
          method: "PATCH",
          headers: getAuthHeaders(),
          body: JSON.stringify({
      ...
      const updated: ApiApplication = await res.json();
      setApplications((prev) =>
        prev.map((app) => (app.id === statusApp.id ? mapApiApp(updated) : app)),
      );
  ...
  const deleteApplication = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/applications/${deletingApp.id}`,
        {
          method: "DELETE",
          headers: getAuthHeaders(),
        },
      ...
      setApplications((prev) =>
        prev.filter((app) => app.id !== deletingApp.id),
      );
      ...
      >
        ...
        <div style={{ marginBottom: 4 }}>
          <h1 style={{ fontSize: isMobile ? 20 : 24, fontWeight: 800 }}>
            Welcome Again, {greeting} 👋
          </h1>
          <p style={{ fontSize: 13.5, color: "var(--pg-muted)", marginTop: 4 }}>
        ...
        >
          <StatCard
            ...
            iconBg="#e7f5ff"
            value={stats.total}
            badge={stats.total > 0 ? "active" : "none yet"}
            badgeType={stats.total > 0 ? "up" : "neutral"}
          ...
          <StatCard
            ...
            badge={stats.interviewing > 0 ? "active" : "none"}
            badgeType={stats.interviewing > 0 ? "up" : "neutral"}
            sub="Currently active pipelines"
          />
          <StatCard
            ...
            iconBg="#f3f0ff"
            value={stats.offers}
            badge={stats.offers > 0 ? "🎉" : "keep going"}
            badgeType={stats.offers > 0 ? "up" : "neutral"}
            sub="Offers in your pipeline"
          />
        </div>
        ...
        >
                    ...
                    style={{
                      ...
                      border:
                        activeFilter === f.value
                          ? `1.5px solid ${FILTER_ACTIVE_COLORS[f.value]}`
                          : "1.5px solid transparent",
                      background:
                        activeFilter === f.value
                          ? FILTER_ACTIVE_COLORS[f.value]
                          : "var(--pg-chip)",
                      color:
                        activeFilter === f.value ? "white" : "var(--pg-muted)",
                      fontFamily: "inherit",
                    }}
                  >
                    {f.label}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    >
                      <div
                        style={{
                          height: "100%",
                          width: `${pct}%`,
                          background: FILTER_ACTIVE_COLORS[f.value],
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg, #3b5bdb, #4c6ef5, #748ffc)",
                borderRadius: 14,
                padding: 20,
                color: "white",
              }}
            >
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: `${Math.min(100, (stats.total / 10) * 100)}%`,
                    background: "white",
                  }}
                />
              </div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          a.href = url;
                          a.download = `optimized-${r.originalFileName}.pdf`;
                          a.click();
                        } catch (e) {
                        } finally {
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <input
                ...
                value={addForm.jobLink}
                onChange={(e) =>
                  setAddForm({ ...addForm, jobLink: e.target.value })
                }
                style={{
                  padding: 10,
                  borderRadius: 8,
              ...
              <input
                ...
                onChange={(e) =>
                  setAddForm({ ...addForm, appliedDate: e.target.value })
                }
                style={{
                  padding: 10,
                  borderRadius: 8,
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {(
                ["applied", "interviewing", "offer", "rejected"] as StatusType[]
              ).map((s) => (
                <button
                  key={s}
      ...
      {showDeleteModal && deletingApp && (
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 13 }}>
              Permanently remove {deletingApp.company} application?
            </p>
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              <button
